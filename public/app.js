...
function lastSeen(value) {
  ...
  if (diff < 3_600_000) return `Last seen ${Math.max(1, Math.floor(diff / 60_000))} min ago`;
...
function setAuthMode(register) {
  ...
  $("login").placeholder = register ? "you@example.com" : "you@example.com or username";
  $("authSubmit").innerHTML = register ? "<span>Create account</span><span aria-hidden=\"true\">→</span>" : "<span>Sign in</span><span aria-hidden=\"true\">→</span>";
  ...
  $("authDescription").textContent = register ? "Create your account, then verify your email." : "Continue your conversations.";
  authError("");
}
...
function showVerification(email) {
  ...
  $("authDescription").textContent = "Enter the 6-digit code we sent you to finish creating your account.";
    ...
    if (registerMode) {
      ...
      if (!response.ok) {
        if (data.code === "EMAIL_NOT_VERIFIED" && data.email) {
          ...
          verificationError("This account already needs email verification. Enter the code or request a new one.");
          return;
        }
...
async function verifyEmail(event) {
  ...
  try {
    ...
    verificationError("Could not connect. Check your connection and try again.");
  } finally {
    submit.disabled = false;
...
function updateChat(username, message, at, user) {
  let chat = chats.find(item => item.user.username.toLowerCase() === username.toLowerCase());
  if (!chat) {
    chat = { conversationId: activeConversationId, user: user || { username }, lastMessage: "", lastMessageAt: at };
  }
...
function appendMessage(message, scroll = true) {
  const mine = message.senderUsername.toLowerCase() === me.username.toLowerCase();
  const messages = $("messages");
  const previous = messages.querySelector(".message:last-of-type");
  const needsDivider = !previous || previous.dataset.dateKey !== dateKey(message.createdAt);
...
function connect() {
  ...
  socket.on("chat:history", data => {
    ...
    updateChat(activeOtherUsername, data.messages.at(-1)?.text || "", data.messages.at(-1)?.createdAt || new Date().toISOString(), data.otherUser);
  ...
  socket.on("chat:message", message => {
    const fromMe = message.senderUsername.toLowerCase() === me.username.toLowerCase();
    const fromActive = message.senderUsername.toLowerCase() === activeOtherUsername?.toLowerCase();
    if (fromMe || fromActive) appendMessage(message);

    const chatUsername = fromMe ? activeOtherUsername : message.senderUsername;
    if (chatUsername) updateChat(chatUsername, message.text, message.createdAt, fromMe ? null : { username: message.senderUsername, name: message.senderName });
    if (!fromMe && !fromActive) {
      const sender = chats.find(item => item.user.username.toLowerCase() === message.senderUsername.toLowerCase())?.user || { username: message.senderUsername, name: message.senderName };
      toast(`New message from ${displayName(sender)}`);
    }
  });

  socket.on("chat:updated", data => {
    updateChat(data.fromUsername, data.lastMessage, data.lastMessageAt, { username: data.fromUsername, name: data.fromName });
    if (data.fromUsername.toLowerCase() !== activeOtherUsername?.toLowerCase()) {
      const sender = chats.find(item => item.user.username.toLowerCase() === data.fromUsername.toLowerCase())?.user || { username: data.fromUsername, name: data.fromName };
      toast(`New message from ${displayName(sender)}`);
    }
  });

  socket.on("presence:update", data => {
    const chat = chats.find(item => item.user.username.toLowerCase() === data.username.toLowerCase());
    if (chat) {
    ...
    if (data.username.toLowerCase() === activeOtherUsername?.toLowerCase()) {
      ...
      typing.textContent = data.online ? "Online now" : lastSeen(typing.dataset.lastSeen);
    }
  });

  socket.on("chat:typing", data => {
    if (data.fromUsername.toLowerCase() !== activeOtherUsername?.toLowerCase()) return;
    const typing = $("typingText");
    typing.textContent = data.isTyping
      ? "Typing…"
      : typing.dataset.online === "1"
        ? "Online now"
        : (typing.dataset.lastSeen ? lastSeen(typing.dataset.lastSeen) : "Private conversation");
  });
}
...
function openChat(username) {
  ...
  const chatUser = chats.find(item => item.user.username.toLowerCase() === username.toLowerCase())?.user;
  $("headerName").textContent = displayName(chatUser || { username });
  $("headerHandle").textContent = handle(chatUser || { username });
  $("headerAvatar").textContent = initials(displayName(chatUser || { username }));
...
async function search() {
  ...
  try {
    ...
    result.classList.remove("hidden");
    result.querySelectorAll(".search-user").forEach(button => button.addEventListener("click", () => {
...
$("emptyNewChat").addEventListener("click", () => $("searchInput").focus());
$("myUsername").addEventListener("click", () => navigator.clipboard?.writeText(me.name || me.username).then(() => toast("Name copied")).catch(() => toast(me.name || me.username)));
...
document.addEventListener("click", event => {
  if (!event.target.closest(".search-section")) $("searchResult").classList.add("hidden");
});
